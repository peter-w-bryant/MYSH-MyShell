...
int main(int argc, char *argv[])
{
        ...
        while ((nread = getline(&line, &nread, interactive ? stdin : batch_fp)) != -1)
        {       
                ...
                if (z == 1)
                {
                        ...
                        while (line[i])
                        {
                                if (!isspace(line[i]) && line[i] != '>')
                                        prog = 1;
                                if (redirection == 1 && line[i] == '>')
                                        exec_program = 0;
                                if (redirection && !isspace(line[i]))
                                        file = 1;
                                if (line[i] == '>')
                                        redirection = 1;
                                if (file == 1 && (isspace(line[i]) || line[i] == '\0'))
                                        file_done = 1;

                                if (prog == 0 && redirection == 1)
                                        exec_program = 0;
                                if (file_done && !isspace(line[i]))
                                        exec_program = 0;
                        ...
                        // Executing Commands
                        if (exec_program)
                        { // Check if the command is valid, lexical check and check for exit also, this entire if needs to be inside both interactive/batch mode
                                ...
                                if (strcmp(command_parse[0], "alias") == 0)
                                {
                                        ...
                                        { // Print for that alias name if exists
                                                AliasListIterator = AliasList;
                                                while (AliasListIterator)
                                                {
                                        ...
                                        { // Add alias
                                                ...
                                                { // Screw this saying "alias: Too dangerous to alias this"
                                        ...
                                        else
                                        {
                                                if (AliasList != NULL)
                                                {
                                                        ...
                                                        do
                                                        {
                                                                ...
                                                                {
                                                                        if (i == 1)
                                                                                TempItem->next = AliasListIterator->next;
                                                                        if (i == 0 && AliasListIterator->next == NULL)
                                                                                AliasList = NULL;
                                                                        if (i == 0 && AliasListIterator->next != NULL)
